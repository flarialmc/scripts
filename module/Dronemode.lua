...
-- Posição e orientação da câmera (drone, controlada por WASD)
local camX, camY, camZ = 0, 64, 0
local centerX, centerY, centerZ = 0, 64, 0
local yaw, pitch = 0, 0
local camInitialized = false

-- Posição real do jogador, cacheada no TickEvent.
-- IMPORTANTE: a doc (events.lua) diz que acessar funcoes de player dentro do
-- RenderEvent NAO e seguro. O original chamava player.position() 2x por frame
-- direto no RenderEvent - provavel causa dos crashes. Agora so lemos aqui.
...
-- ponytail: o raycast (o custo pesado) so e recalculado no maximo a essa taxa.
-- O desenho (drawList:AddRectFilled) continua rodando todo frame usando a ultima
-- cor calculada, entao nao ha flicker. Sobe a taxa se tiver CPU sobrando.
local RAYCAST_INTERVAL = 1 / 30
local lastRaycastTime = os.clock()

-- Pool de retângulos: criado 1 vez, tabelas de posição nunca realocadas depois
local buttonPool = {}
...
local keyMap = {
    ...
    -- 17 = LWJGL KEY_W, ja mapeado acima; nao pode reusar pra ctrl (bug no original,
    -- fazia [17]="w" ser sobrescrito por [17]="ctrl" e "w" nunca disparava nesse esquema)
    [29] = "ctrl",                    -- Ctrl Esquerdo
...
local RadiusSlider = settings.addSlider("Radius (blocks)","The radius for scan in blocks, recommended: 30", 30, 100, 5)
local BlockPerSecondSlider = settings.addSlider("Blocks per second", "Blocks per second for scan, recommended: 50000", 5000, 20000, 200)
...
-- TickEvent: ÚNICO lugar que le player.*/world.* (RenderEvent nao e seguro pra isso,
-- ver https://scripting.flarial.xyz/api/game/events). Roda a 20/s, sobra de sobra
-- pra manter a posicao do jogador e o escaneamento em dia.
...
-- RenderEvent: só desenha. Nenhuma chamada a player.*/world.* aqui.
onEvent("RenderEvent", function()
    local drawList = ImGui.GetForegroundDrawList()

    -- Controles do drone (não toca em player.*/world.*, seguro no RenderEvent)
    if renderVisible then
    ...
    if not buttonsInitialized then
        for py = 0, H do
            buttonPool[py] = {}
            for px = 0, W do
                local x1, y1 = px * 15, py * 15
                buttonPool[py][px] = {
                    -- Tabelas de posição alocadas 1 vez só, nunca mais recriadas
                    p1 = { x1, y1 },
                    p2 = { x1 + 40, y1 + 40 },
    ...
    local currentTime = os.clock()
    local doRaycast = (currentTime - lastRaycastTime) >= RAYCAST_INTERVAL
    if doRaycast then lastRaycastTime = currentTime end

    -- Raycasting 3D. O calculo pesado só roda quando doRaycast é true; o desenho
    -- (barato) roda sempre, usando a última cor calculada.
    for py = 0, H do
        local rowDirs = rayDirections[py]
        local rowButtons = buttonPool[py]
        for px = 0, W do
            local btn = rowButtons[px]
            ...
            drawList:AddRectFilled(btn.p1, btn.p2, btn.color, 0, 0)
        end
    end
end)
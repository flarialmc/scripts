...
-- Estado de renderização e escaneamento
...
-- Sistema anti-spam para teclas configuráveis
local keyCooldowns = {
    ui = 0,
    rescan = 0,
    teleport = 0
}
local KEY_COOLDOWN_TIME = 1.0

-- Configurações principais
local RADIUS = 15
local BLOCKS_PER_SECOND = 3000

-- Cache de funções matemáticas (otimização)
...
-- Gera fila de escaneamento em espiral concêntrica
local function generateScanQueue(radius)
    local queue = {}
    for r = 0, radius do
        for x = -r, r do
            for y = -r, r do
                for z = -r, r do
                    if math.abs(x) == r or math.abs(y) == r or math.abs(z) == r then
...
local keyMap = {
    -- LETRAS: W, A, S, D (Movimento)
    [87] = "w", [17] = "w",   -- W
    [65] = "a", [30] = "a",   -- A
    [83] = "s", [31] = "s",   -- S
    [68] = "d", [32] = "d",   -- D
    
    -- OUTRAS LETRAS (Backup)
    [90] = "z", [44] = "z",   -- Z
    [67] = "c", [46] = "c",   -- C
    [88] = "x", [45] = "x",   -- X
    
    -- MODIFICADORES
    [32] = "space",           -- Espaço (ASCII)
    [57] = "space",           -- Espaço (LWJGL)
    
    [16] = "shift", [42] = "shift",   -- Shift Esquerdo
    [54] = "shift",                   -- Shift Direito
    [340] = "shift", [344] = "shift", -- Shift (GLFW)
    
    [17] = "ctrl", [29] = "ctrl",     -- Ctrl Esquerdo
    [157] = "ctrl",                   -- Ctrl Direito
    [341] = "ctrl", [345] = "ctrl",   -- Ctrl (GLFW)
    [162] = "ctrl", [163] = "ctrl",   -- Ctrl (Windows VK)
    
    -- SETAS: Movimento da Câmera
    [265] = "up", [38] = "up", [200] = "up",       -- Seta Cima
    [264] = "down", [40] = "down", [208] = "down", -- Seta Baixo
    [263] = "left", [37] = "left", [203] = "left", -- Seta Esquerda
    [266] = "right", [39] = "right", [205] = "right" -- Seta Direita
}

-- ==========================================
-- 4. CONFIGURAÇÕES DA UI
-- ==========================================

settings.addHeader("Configure drone")
settings.addHeader("BE CAREFUL OR IT WILL CRASH YOUR MINECRAFT")

local RadiusSlider = settings.addSlider("Radius (blocks)","The radius for scan in blocks, max recommended: 30", 15, 40, 5)
local BlockPerSecondSlider = settings.addSlider("Blocks per second", "Blocks per second for scan, max recommended: 5000", 3000, 10000, 200)

local OpenDroneUIKey = settings.addKeybind("Open Drone UI", "Open drone UI (open inventory and press the key)", "o")
local ReescanKey = settings.addKeybind("Rescan", "Rescan Area", "u")
local TeleportToPlayerKey = settings.addKeybind("TP to player", "Teleport your camera to the player and reset scan", "i")

-- Reseta o cache e reinicia o escaneamento
local function resetCache()
    -- BUG FIX 3: Aplica configurações pendentes antes de resetar
    local newRadius = m_floor(tonumber(RadiusSlider.value))
    local newBLOCKS_PER_SECOND = m_floor(tonumber(BlockPerSecondSlider.value))
    
    if newRadius ~= RADIUS then
        RADIUS = newRadius
    end
    
    if newBLOCKS_PER_SECOND ~= BLOCKS_PER_SECOND then
        BLOCKS_PER_SECOND = newBLOCKS_PER_SECOND
    end
    
    worldCache = {}
    
    centerX = m_floor(camX)
    centerY = m_floor(camY)
    centerZ = m_floor(camZ)
    
    scanQueue = generateScanQueue(RADIUS)
    TOTAL_BLOCKS = #scanQueue
    
    scanIndex = 1
    lastScanTime = os.clock()
    scanCompleted = false

    client.notify("Cache cleared! Radius: " .. RADIUS .. " | Restarting scan...")
end

local function canUseKey(keyName)
    local currentTime = os.clock()
    if currentTime - keyCooldowns[keyName] >= KEY_COOLDOWN_TIME then
        keyCooldowns[keyName] = currentTime
        return true
    end
    return false
end
...
for py = 0, H do
    rayDirections[py] = {}
    for px = 0, W do
        local u = (px - centerScreenX) / halfW
        local v = (centerScreenY - py) / halfH
        rayDirections[py][px] = {u * fovX, v * fovY, 1.0}
    end
end

-- Pool de botões (cria 1 vez, atualiza sempre)
local buttonPool = {}
...
onEvent("KeyEvent", function(key, action)
    if action == 1 then
        local isUIKey = OpenDroneUIKey.value
        local isRescanKey = ReescanKey.value
        local isTPKey = TeleportToPlayerKey.value
        
        if isUIKey and canUseKey("ui") then
            if not scanCompleted and renderVisible then
                client.notify("Wait for the scan to finish before rescanning.")
            else
                -- Apenas marca flag para aplicar configurações no próximo frame (evita crash)
                renderVisible = not renderVisible
            end

        elseif isRescanKey and canUseKey("rescan") then
        ...
        elseif isTPKey and canUseKey("teleport") then
            if scanCompleted and renderVisible then
                ...
                if pX then
                    ...
                    client.notify("Camera teleported to player!")
                end
                resetCache()
            end
        end
...
-- Handler do Mouse (Scroll para FOV)
onEvent("MouseEvent", function(button, action)
    if button == 4 then
        if action == 4 then
            fovY = fovY + 0.1
            if fovY > 1.8 then fovY = 1.8 end
        elseif action == 3 then
            fovY = fovY - 0.1
            if fovY < 0.1 then fovY = 0.1 end
        end
        fovX = fovY * (W / H)
...
-- ==========================================
-- 8. LOOP PRINCIPAL (RENDEREVENT)
-- ==========================================

onEvent("RenderEvent", function()
    local scanInterval = 1.0 / BLOCKS_PER_SECOND

    -- A. Inicializar Câmera
    if not camInitialized then
        local px, py, pz = player.position()
        if px then
            centerX, centerY, centerZ = m_floor(px), m_floor(py), m_floor(pz)
            camX, camY, camZ = px, py + 1.6, pz
            camInitialized = true
        end
    end

    -- B. Processar Escaneamento
    local currentTime = os.clock()
    local timeSinceLastScan = currentTime - lastScanTime
    local blocksToScan = m_floor(timeSinceLastScan / scanInterval)

    if blocksToScan > 0 and scanIndex <= TOTAL_BLOCKS then
        lastScanTime = currentTime
        local endIdx = m_min(scanIndex + blocksToScan, TOTAL_BLOCKS + 1)
        for i = scanIndex, endIdx - 1 do
            local offset = scanQueue[i]
            local wx = centerX + offset[1]
            local wy = centerY + offset[2]
            local wz = centerZ + offset[3]
            
            local blockName = world.getBlock(wx, wy, wz)
            if not worldCache[wx] then worldCache[wx] = {} end
            if not worldCache[wx][wy] then worldCache[wx][wy] = {} end
            worldCache[wx][wy][wz] = blockName or "minecraft:air"
        end
        scanIndex = endIdx
    end

    if not scanCompleted and scanIndex > TOTAL_BLOCKS then
        scanCompleted = true
        client.notify("Scan complete!")
    end

    -- C. Processar Controles
    if renderVisible then
        if keys["shift"] then moveSpeed = 0.8
        elseif keys["z"] then moveSpeed = 0.05
        else moveSpeed = baseSpeed end

        if keys["w"] then camX = camX + m_sin(yaw) * moveSpeed; camZ = camZ - m_cos(yaw) * moveSpeed end
        if keys["s"] then camX = camX - m_sin(yaw) * moveSpeed; camZ = camZ + m_cos(yaw) * moveSpeed end
        if keys["a"] then camX = camX - m_cos(yaw) * moveSpeed; camZ = camZ - m_sin(yaw) * moveSpeed end
        if keys["d"] then camX = camX + m_cos(yaw) * moveSpeed; camZ = camZ + m_sin(yaw) * moveSpeed end
        
        if keys["space"] then camY = camY + moveSpeed end
        if keys["c"] then camY = camY - moveSpeed end

        local rotSpeed = 0.08
        if keys["left"] then yaw = yaw - rotSpeed end
        if keys["right"] then yaw = yaw + rotSpeed end
        if keys["up"] then pitch = pitch - rotSpeed end
        if keys["down"] then pitch = pitch + rotSpeed end
    ...
    local pX, pY, pZ = player.position()
    local fpX, fpY, fpZ = m_floor(pX), m_floor(pY), m_floor(pZ)
    ...
    if renderVisible then
        -- Inicializa pool de botões na primeira vez
        ...
        -- Pré-calcula posição da câmera UMA VEZ fora do loop
        local camMapX, camMapY, camMapZ = m_floor(camX), m_floor(camY), m_floor(camZ)
        
        -- Pré-calcula limites do mundo usando RADIUS
        local worldMinX = centerX - RADIUS
        local worldMaxX = centerX + RADIUS
        local worldMinY = centerY - RADIUS
        local worldMaxY = centerY + RADIUS
        local worldMinZ = centerZ - RADIUS
        local worldMaxZ = centerZ + RADIUS

        -- D. Raycasting 3D
        for py = 0, H do
            for px = 0, W do
                
                -- Usa direção pré-calculada
                local rd = rayDirections[py][px]
                local rx, ry, rz = rd[1], rd[2], rd[3]
                
                local ry2 = ry * cosP - rz * sinP
                local rz2 = ry * sinP + rz * cosP
                
                local rx3 = rx * cosY + rz2 * sinY
                local rz3 = -rx * sinY + rz2 * cosY 
                
                -- Inverte o eixo Z para alinhar com o Minecraft (Z- é Norte/Frente)
                local dirX, dirY, dirZ = rx3, ry2, -rz3
                ...
                if dirX < 0 then stepX = -1; sideDistX = (camX - mapX) * deltaDistX
                else stepX = 1; sideDistX = (mapX + 1 - camX) * deltaDistX end
                
                if dirY < 0 then stepY = -1; sideDistY = (camY - mapY) * deltaDistY
                else stepY = 1; sideDistY = (mapY + 1 - camY) * deltaDistY end
                
                if dirZ < 0 then stepZ = -1; sideDistZ = (camZ - mapZ) * deltaDistZ
                else stepZ = 1; sideDistZ = (mapZ + 1 - camZ) * deltaDistZ end
                
                local hit, side = 0, 0
                local steps = 0
                
                -- Limite de 60 passos
                while hit == 0 and steps < 60 do
                    steps = steps + 1
                    ...
                    elseif sideDistY < sideDistZ then
                        ...
                        sideDistZ = sideDistZ + deltaDistZ; mapZ = mapZ + stepZ; side = 2
                    end
                    ...
                    -- Usa comparações simples ao invés de m_abs
                    ...
                    if inBounds then
                        local cachedX = worldCache[mapX]
                        local isScanned = cachedX and cachedX[mapY] and cachedX[mapY][mapZ] ~= nil
                        
                        if isScanned then
                            local cachedBlock = cachedX[mapY][mapZ]
                            
                            if cachedBlock and cachedBlock ~= "minecraft:air" then
                                local nx, ny, nz = mapX, mapY, mapZ
                                if side == 0 then nx = mapX - stepX
                                elseif side == 1 then ny = mapY - stepY
                                elseif side == 2 then nz = mapZ - stepZ
                                end
                                
                                local neighborInBounds = (nx >= worldMinX and nx <= worldMaxX and 
                                                          ny >= worldMinY and ny <= worldMaxY and 
                                                          nz >= worldMinZ and nz <= worldMaxZ)
                                
                                if not neighborInBounds then
                                    hit = 1
                                else
                                    local nCachedX = worldCache[nx]
                                    local nBlock = nCachedX and nCachedX[ny] and nCachedX[ny][nz]
                                    
                                    if not nBlock or nBlock == "minecraft:air" then
                                        hit = 1
                                    else
                                        hit = 0
                                    end
                ...
                -- E. Obter Cor
                local r, g, b, a = 150, 50, 200, 0
                
                if hit == 1 then
                    local cachedX = worldCache[mapX]
                    local blockName = cachedX and cachedX[mapY] and cachedX[mapY][mapZ]
                    local color = blockName and blockColors[blockName]
                    
                    if color then
                        if side == 0 then
                            r = m_floor(color[1] * 0.8)
                            g = m_floor(color[2] * 0.8)
                            b = m_floor(color[3] * 0.8)
                        elseif side == 1 then
                            r = m_floor(color[1] * 0.9)
                            g = m_floor(color[2] * 0.9)
                            b = m_floor(color[3] * 0.9)
                        else
                            r = m_floor(color[1] * 0.6)
                            g = m_floor(color[2] * 0.6)
                            b = m_floor(color[3] * 0.6)
                        end
                    end
                    a = 255
                elseif hit == 4 then
                ...
                -- Atualiza o botão no pool (sem criar novo)
                local btn = buttonPool[py][px]
                btn.color[1] = r
                btn.color[2] = g
                btn.color[3] = b
                btn.color[4] = a
                
                -- Desenha o botão usando o pool
                gui.button(btn.x*1.5, btn.y*1.5, btn.color, btn.color, "", 40, 40)
            end
        end
    end
end)